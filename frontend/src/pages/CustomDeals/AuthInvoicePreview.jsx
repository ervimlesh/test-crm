...
const AuthInvoicePreview = () => {
  ...
  const { flightDetails, isAuthFlow, flightId, newRemark } = state || {};
  ...
  try {
    ...
    if (response.data.success) {
      ...
      const activeAuthMail = allMails.filter(
        (mail) => mail.status === "active" && mail.mailStatus === "auth-mail"
      );
...
useEffect(() => {
  // If nothing is selected → pick the first ACTIVE auth mail
  if (!selectedAuthMail && authMails.length > 0) {
    setSelectedAuthMail(authMails[0]);
  }
}, [authMails]);



  // const bookingId = `BID-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    const fetchLastBooking = async () => {
      try {
        ...
        if (last) {
          ...
          console.log("upcoing m is ", m);
          if (m && m[1]) {
            newId = isAuthFlow
              ? `BID-${m[1]}`
              : `BID-${parseInt(m[1], 10) + 1}`;

            newId = `BID-6583`;
          }
        } else {
          // no previous booking found -> start with BID-6583
          newId = `BID-6583`;
        }
        if (mounted) setBookingId(newId);
      } catch (e) {
        console.error("Error fetching last bookingId", e);
        if (mounted) setBookingId(`BID-6583`);
      }
    };
  ...
  const getProviderInfo = () => {
    // If provider is an object with provider field, it's coming from DetailsPnrFlight
    if (flightDetails?.provider?.provider) {
      console.log("herere", flightDetails);
      return {
        name: flightDetails.provider.provider,
        tollFreePrimary: flightDetails.provider.tollFreePrimary,
        tollFreeSecondary: flightDetails.provider.tollFreeSecondary,
        originalProvider: flightDetails.provider,
      };
    }

    // If provider is a string and we have tollFreeNumber array, it's coming from createCtmFlight
    else if (flightDetails?.provider && flightDetails?.tollFreeNumber) {
      const tollProvider = flightDetails.tollFreeNumber.find(
        (f) => f.provider === flightDetails.provider
      );
      return {
        name: flightDetails.provider,
        tollFreePrimary: tollProvider?.tollFreePrimary,
        tollFreeSecondary: tollProvider?.tollFreeSecondary,
        originalProvider: tollProvider,
      };
    }
    // Fallback
    return {
      name: flightDetails?.provider || "N/A",
      tollFreePrimary: "N/A",
      tollFreeSecondary: "N/A",
  ...
  const tollNumber =
    language === "en"
      ? providerInfo.tollFreePrimary
      : providerInfo.tollFreeSecondary;
  ...
  //       language,
  //       tollNumber,
  //       provider: providerInfo.originalProvider || providerInfo.name,
  //     };
  //     await axios.post("/api/v1/ctmFlights/create-ctm-flight", payload);
  //     try {
  //       sessionStorage.removeItem("ctm_flightDetails");
  //     } catch (e) {
  //       console.log("Could not clear ctm_flightDetails from sessionStorage", e);
  //     }
    ...
    try {
      if (isAuthFlow) {
        ...
        const res = await axios.patch(
          ...
          { pnrRemark: newRemark, authCredentials }
        );
        ...
        console.log("its is callin now for else");
        const payload = {
          ...flightDetails,
          bookingId,
          language,
      ...
      console.error(error);
      alert("Error submitting authorization form.");
    }
  };
                      ...
                      >
                        ...
                        >
                          ...
                          <div style={{ display: "flex", gap: "5px" }}>
                            {/* <p>{providerInfo.tollFreePrimary} | </p>
                                <p>{providerInfo.tollFreeSecondary}</p> */}
                          </div>
                        </a>
                      </li>
                    </ul>
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    <b>{providerInfo.name}</b> to charge my Debit/Credit card
                    for{" "}
                    <b style={{ textTransform: "capitalize" }}>
                      {/* {Number(flightDetails?.baseFare) +
                        Number(flightDetails?.taxes)} */}
                      {(() => {
                        const base = Number(flightDetails?.baseFare ?? 0);
                        const tax = Number(flightDetails?.taxes ?? 0);
                        const total = base + tax;

                        // Preserve decimal precision based on inputs
                        const baseDec = (
                          flightDetails?.baseFare?.toString().split(".")[1] ||
                          ""
                        ).length;
                        const taxDec = (
                          flightDetails?.taxes?.toString().split(".")[1] || ""
                        ).length;
                        const maxDec = Math.max(baseDec, taxDec);

                        return total.toFixed(maxDec);
                      })()}
                    </b>
                    <b style={{ textTransform: "uppercase" }}>
                      {flightDetails?.currency}
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      {/* <b>
                       {providerInfo.tollFreePrimary} | {providerInfo.tollFreeSecondary}
                      </b> */}
                    </li>
                    <li
                      style={{
                ...
                >
                  <button
                    ...
                    onClick={() => {
                      if (isAuthFlow) {
                        navigate(`/astrivion/details-pnr-flight/${flightId}`, {
                          state: { flightDetails },
                        });
                      } else {
                        navigate("/astrivion/create-flight-booking", {
...
export default AuthInvoicePreview;

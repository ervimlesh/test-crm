...
const CreateCtmship = () => {
  ...
  const handleChange = (e) => {
    ...
    // Handle outbound/inbound segment array creation on Ship count selection
    if (name === "outboundShip") {
  ...
  const validateStep1 = () => {
    ...
    if (formData.itineraryType === "roundTrip") {
      if (
        !formData.inboundShip ||
        !formData.inboundShip.toString().trim()
      ) {
        toast.error("Please fill the Inbound Ship");
        return false;
      }
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      for (const field of requiredSegmentFields) {
        if (!segment[field] || segment[field].toString().trim() === "") {
          toast.error(
            `Please fill ${field} for ${type} ship segment ${i + 1}`
          );
          return false;
        }
      }
  ...
  useEffect(() => {
    // Try to restore ship details either from location.state (when coming
    // back explicitly with state) or from sessionStorage (fallback).
    console.log("attempting to restore saved ship details");

    const restoreFromLocation = () => {
      if (location.state?.shipDetails) {
        const { shipDetails } = location.state;
        console.log("restoring from location.state", location.state);

        setFormData((prev) => ({ ...prev, ...shipDetails }));
        setOutboundSegments(shipDetails.outboundSegments || []);
        setInboundSegments(shipDetails.inboundSegments || []);
        setPassengerDetails(shipDetails.passengerDetails || []);
        // If segments were present previously, ensure we return to step 2
        if (
          location.state.step ||
          (shipDetails.outboundSegments &&
            shipDetails.outboundSegments.length > 0) ||
          (shipDetails.inboundSegments &&
            shipDetails.inboundSegments.length > 0)
        ) {
    ...
    const restoreFromSession = () => {
      try {
        const raw = sessionStorage.getItem("ctm_shipDetails");
        if (!raw) return false;
        const parsed = JSON.parse(raw);
        const { shipDetails: fd, step: savedStep } = parsed || {};
        if (!fd) return false;
        ...
        console.warn("failed to parse session ctm_shipDetails", e);
        return false;
      }
    };
  ...
  const handleRefreshClick = () => {
    sessionStorage.removeItem("ctm_shipDetails");
                ...
                {step === 1 ? (
                  <div className="">
                    ...
                    <div className="row">
                      ...
                      {/* Outbound Ship Stop Count */}
                      {(formData.itineraryType === "oneWay" ||
                        formData.itineraryType === "roundTrip") && (
                        <div className="col-12 mt-4">
                          <div className="form_travel">
                            <label htmlFor="outboundShip">
                              Outbound Ship
                            </label>
                            <select
                              name="outboundShip"
                              className="form-select form_all"
                              value={formData.outboundShip}
                              onChange={handleChange}
                              required
                            >
                              <option value="">Select</option>
                      ...
                      {/* Inbound Ship Stop Count */}
                      {formData.itineraryType === "roundTrip" && (
                        <div className="col-12 mt-4">
                          <div className="form_travel">
                            <label htmlFor="inboundShip">
                              Inbound Ship
                            </label>
                            <select
                              name="inboundShip"
                              className="form-select form_all"
                              value={formData.inboundShip}
                              onChange={handleChange}
                              required
                            >
                              <option value="">Select</option>
                        ...
                        {outboundSegments.map((segment, index) => (
                          ...
                          >
                            ...
                            <div key={index} className="flex_all form_box_auth">
                              ...
                              <div className="width_control_in13 box_sp">
                                <div className="box_crm_input">
                                  <label htmlFor={`shipno_send${index}`}>
                                    Ship No.
                                  </label>
                                  <input
                                    placeholder="Ship"
                                    id={`shipno_send${index}`}
                                    value={segment.ship || ""}
                                    onChange={(e) =>
                                      handleSegmentChange(
                                        "outbound",
                              ...
                              >
                                ...
                                <div className="flex_all form_box_auth">
                                  ...
                                  <div className="width_control_in13 box_sp">
                                    <div className="box_crm_input">
                                      <label
                                        htmlFor={`sno_inbound${index}`}
                                      >
                                        s No.
                                      </label>
                                      <input
                                        placeholder="s"
                                        id={`sno_inbound${index}`}
                                        value={segment.s || ""}
                                        onChange={(e) =>
                                          handleSegmentChange(
                                            "inbound",
